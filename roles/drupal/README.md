...
Git is not an absolute requirement, but is required if you're deploying from a Git repository (e.g. `drupal_deploy: true`). You can use `startcloud.startcloud_roles.git` to install Git.
...
See the example playbooks used for Travis CI tests (`tests/test.yml` and `tests/test-deploy.yml`) for simple examples. See also: [Drupal VM](https://www.drupalvm.com), which uses this role to set up Drupal.
    ...
    - hosts: webserver
      ...
      roles:
        ...
        - startcloud.startcloud_roles.php-versions
        - startcloud.startcloud_roles.php
        - startcloud.startcloud_roles.php-mysql
        - startcloud.startcloud_roles.composer
        - startcloud.startcloud_roles.drush
